...
const turn = (n: number, role: "user" | "assistant", content: string): Turn => ({
  version: "scroll/0.1", turn: n, role,
  model: { vendor: "demo", id: "v0" },
  params: { temperature: 0, top_p: 1 },
  messages: [{ role, content }],
  timestamp_ns: n,
});
const sealed = await sealChain([turn(0, "user", "hello"), turn(1, "assistant", "hi back")], { privkey, pubkey });
console.log("canonical turn 0:", sealed[0] ? new TextDecoder().decode(canonical(sealed[0])) : "(empty)");
